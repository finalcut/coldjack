...
component hint="runs some MXUnit Tests on the coldjack component" output="false" extends="mxunit.framework.testcase" {
  ...
  public void function testReadSimpleDb(){
    ...
    assertEquals(3,q.recordCount,'rowcount');
    ...
    assertEquals("lived a young jedi",q.story[rownum]);
    // bio in the db is a "memo" so it has some built in formatting which is
    // where the <div> comes from
    assertEquals("<div>a long time ago in a galaxy far far away</div>",q.bio[rownum]);
    ...
    rownum = 3;
    assertEquals("Abe",q.firstName[rownum]);
    assertEquals(0,q.subscribe[rownum]);
    assertEquals(500000.00,q.salary[rownum]);
    assertEquals("##http://whitehouse.gov##",q.website[rownum]);
    assertEquals("he was assassinated at a play",q.story[rownum]);
  ...
  public void function testReadWithSimpleColumnMap(){
    coldjack = createObject("component", "coldjack");
    db = getDirectoryFromPath(getCurrentTemplatePath()) & "\test.accdb";
    ...
    // bio in the db is a "memo" so it has some built in formatting which is
    // where the <div> comes from
    assertEquals("<div>a long time ago in a galaxy far far away</div>",q.biography[1]);

    // column wasn't mapped yet!
    assertEquals("10.24",q.chapterSection[1]);
    assertEquals("2.9",q.chapterSection[2]);
    assertEquals("1.20.300.4000",q.chapterSection[3]);

  }

  public void function testReadWithSplitColumnMap(){
    coldjack = createObject("component", "coldjack");
    db = getDirectoryFromPath(getCurrentTemplatePath()) & "\test.accdb";
    ...
    rownum=1;
    assertEquals("10",q.chapter[rownum]);
    assertEquals("24",q.section[rownum]);

    rownum=2;
    assertEquals("2",q.chapter[rownum]);
    assertEquals("9",q.section[rownum]);

    rownum=3;
    assertEquals("1",q.chapter[rownum]);
    assertEquals("20",q.section[rownum]);
  }

  public void function testReadWithSplitColumnMapWithTooManuDestinationColumns(){
    coldjack = createObject("component", "coldjack");
    db = getDirectoryFromPath(getCurrentTemplatePath()) & "\test.accdb";
    ...
    // column will be split into two columns Chapter and Section
    // this means each ChapterSection in the access database will be in the format Chapter.Section
    colMap = {"FirstName":"PersonName","Bio":"Biography","ChapterSection":"Chapter,Section,Regulation|."};
    ...
    assertEquals("10",q.chapter[rownum]);
    assertEquals("24",q.section[rownum]);
    assertEquals("",q.regulation[rownum]);
    ...
    assertEquals("1",q.chapter[rownum]);
    assertEquals("20",q.section[rownum]);
    assertEquals("300",q.regulation[rownum]);
